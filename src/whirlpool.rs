...
impl Digest for Whirlpool {
    fn input(&mut self, source: &[u8]) {
        ...
        self.buffer.input(source, |b| {
            process_buffer(hash, b);
        });
    }
    ...
    fn result(&mut self, out: &mut [u8]) {
        if !self.finalized {
            ...
            let hash = &mut self.hash;
            self.buffer.input(&[0b10000000], |b| {
                process_buffer(hash, b);
            });
            ...
            self.buffer.input(&self.bit_length, |b| {
                process_buffer(hash, b);
            });
