...
impl Blake2s {
    ...
    fn apply_param(&mut self) {
        ...
        let mut param_bytes: [u8; 32] = [0; 32];
        {
            ...
            let mut writer: Vec<u8> = Vec::with_capacity(32);
            writer.write_u8(self.param.digest_length);
            writer.write_u8(self.param.key_length);
            writer.write_u8(self.param.fanout);
            writer.write_u8(self.param.depth);
            writer.write_u32_le(self.param.leaf_length);
            writer.write_all(&self.param.node_offset);
            writer.write_u8(self.param.node_depth);
            writer.write_u8(self.param.inner_length);
            writer.write_all(&self.param.salt);
            writer.write_all(&self.param.personal);
            param_bytes.clone_from_slice(&writer);
        }
